...
<script type="module"
	src="/resources/script/validation-notice-delete.js"></script>
...
<body>
	...
	<main id="write-notice">
		...
		<form
			action="<c:choose>
                  <c:when test="${isEditMode}">/notice/update</c:when>
                  <c:otherwise>/notice/write</c:otherwise>
                </c:choose>"
			method="post">
			<c:if test="${isEditMode}">
				<input type="hidden" id="notice-code" name="code"
					value="${notice.code}"	>
				<input type="hidden" name="regDate" value="${notice.regDate}">
			</c:if>			
			<input type="hidden" name="adminCode" value="${admin.code}">
			<label for="title">제목</label> <input type="text" id="title"
				name="title" value="${notice.title}" required><br> <label
				for="content">내용</label>
			<textarea id="content" name="content" required>${notice.content}</textarea>
			<br> <label for="start-date">게시일</label> <input type="date"
				id="start-date" name="startDate" value="${notice.formattedResDate}"><br>

			<label for="end-date">만료일</label> <input type="date" id="end-date"
				name="endDate" value="${notice.formattedCloseDate}"><br>
